...
class Dispatcher:
    def __init__(
        ...
        cooperative_mode: bool = False,          # 协同模式开关
    ):
        """
        初始化调度中心
        """
    ...
    def _get_scale_config(self, state: dict) -> dict:
        """统一的地图规模推导逻辑，修复 Fallback bug，供 Hybrid 和 GA 共同调用"""
        metrics = state.get('metrics', {})
        map_scale = "medium"
        
        if 'scenario' in state:
            map_scale = str(state.get('scenario', 'medium')).lower()
        elif 'map_name' in state:
            map_scale = str(state.get('map_name', 'medium')).lower()
        else:
            total_tasks = metrics.get('total_tasks', len(state.get('tasks', [])))
            if total_tasks <= 20:
                map_scale = "small"
            elif total_tasks <= 50:
                map_scale = "medium"
                ...
                map_scale = "extra_large"

        # 字符串兜底匹配
        if "small" in map_scale: map_scale = "small"
        elif "extra" in map_scale: map_scale = "extra_large"
        elif "large" in map_scale: map_scale = "large"
        else: map_scale = "medium"
        ...
        scale_config = {
            ...
            "extra_large": {"prune_radius": 1500.0, "soc_trigger": 0.75} # 新增超大规模防崩溃
        }
        return scale_config.get(map_scale, scale_config["medium"])

    def _try_cooperative_assignment(self, available_vehicles: List[dict], task: dict, state: dict) -> Tuple[Optional[List[dict]], Optional[List[int]]]:
        ...
        if remaining_weight > 1e-4:
            return None, None

        return actions, assigned_indices

    def dispatch(self, state: dict) -> list:
        idle_vehicles = [v for v in state.get('vehicles', []) if str(v.get('status', '')).lower() == 'idle']
        unassigned_tasks = [t for t in state.get('tasks', []) if str(t.get('status', '')).lower() == 'waiting']
        ...
        if self.strategy_name == "largest":
            ...
            import strategy_ga
            # 修改点：将 dispatcher 实例(self)直接传给 GA
            return strategy_ga.ga_dispatch(
                idle_vehicles, unassigned_tasks, self.pathfinder, state, self
            )
        else:
            return []
    ...
    def _largest_dispatch(self, idle_vehicles, unassigned_tasks, state) -> list:
        ...
        for vehicle in idle_vehicles:
            if vehicle['id'] in assigned_vehicle_ids: continue
            if len(assigned_task_ids) == len(unassigned_tasks): break

            task = None
            for candidate in unassigned_tasks:
                if candidate['id'] in assigned_task_ids: continue
    ...
    def _nearest_dispatch(self, idle_vehicles, unassigned_tasks, state) -> list:
        ...
        for vehicle in idle_vehicles:
            ...
            best_task, best_path, shortest_dist = None, None, float('inf')
            start_node = int(vehicle.get('current_node', 0))

            for task in unassigned_tasks:
                if task['id'] in assigned_task_ids: continue
    ...
    def _energy_aware_hybrid_dispatch(self, idle_vehicles, unassigned_tasks, state) -> list:
        ...
        # 统一读取配置
        config = self._get_scale_config(state)
        ...
        for vehicle in idle_vehicles:
            ...
            best_task, best_path, max_score = None, None, -float('inf')
            ...
            v_capacity = self._as_float(
                vehicle.get('max_load', vehicle.get('load_capacity', vehicle.get('capacity', self.load_capacity))),
                self.load_capacity,
            )
            ...
            for task in unassigned_tasks:
                ...
                if not (single_ok or coop_ok): continue

                task_node = int(task.get('node_id', 0))
                try:
                    path, d1 = self.pathfinder.find_path_and_distance(veh_node, task_node)
                    if d1 > max_prune_radius: continue
                    ...
                    if d2 == float('inf'): continue

                    time_left = task.get('deadline', 9999) - current_time
                    time_to_reach = d1 / v_speed
                    if time_to_reach >= time_left: continue

                    required_energy = (d1 + d2) * self.consume_rate
                    if veh_battery < required_energy + self.safety_margin: continue
                    ...
                    total_score = self.base_score + profit_score + urgency_score - distance_penalty - battery_risk_penalty - queue_penalty
    ...
    def _get_nearest_charger_info(self, task_node, chargers):
        if not chargers: return None, float('inf'), 0
        min_dist, best_charger = float('inf'), None
