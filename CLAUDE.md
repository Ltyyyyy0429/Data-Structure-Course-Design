...
- **多策略调度**：已实现 `nearest`（最近任务优先）、`largest`（最大任务优先）、`energy_aware_hybrid`（能量感知综合调度）、`genetic_algorithm`（遗传算法调度）四种策略
...
模块 A (core/)         模块 B (simulator/)
  图结构 & Dijkstra  ──►  RealPathfinder (适配层)
             ...
             strategy_ga.py  事件循环    Pygame + Matplotlib
             调度策略(4种)
```
...
├── strategy.py                 # 模块 C: 调度策略 (nearest / largest / energy_aware_hybrid / genetic_algorithm)
├── strategy_ga.py              # 模块 C: 遗传算法调度策略实现
...
```bash
python batch_experiment.py              # 4 策略 × 4 规模 × EASY 难度 = 16 组实验
python batch_experiment.py medium       # 16 组实验，使用 MEDIUM 难度参数
python batch_experiment.py hard         # 16 组实验，使用 HARD 难度参数
```
...
**Dispatcher `consume_rate` 参数**：`Dispatcher.__init__` 接受 `consume_rate` 参数，Simulator 构造 Dispatcher 时传入实际 `energy_per_km` 值（EASY=0.35, MEDIUM=0.60, HARD=0.90），确保策略层的能量硬门槛与实际仿真能耗一致。**P1 修复**：原实现虽接受参数但硬编码为 0.5，现已修复为使用传入值（`None` 时回退 0.5）。
...
**GA 策略与 Hybrid 策略打分公式统一**：`strategy_ga.py` 的适应度函数与 `strategy.py` 的 `_energy_aware_hybrid_dispatch` 使用相同的百分制打分公式——收益归一化用车辆载重上限、紧急度用缓冲时间（扣除赶路时间）、电池惩罚用二次函数。GA 也包含时间可行性（死亡冲锋）预检，与 hybrid 策略的硬约束一致。两个文件的 `_as_float` / `_is_load_feasible` 函数因避免循环导入而各自保留（已标记为 mirror 注释）。
